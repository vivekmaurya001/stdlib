...
for ( i = 0; i < x.length; i++ ) {
	...
	ycustom[ i ] = atanhf( x[ i ] );
	ynative[ i ] = Math.atanh( x[ i ] );
}

customErrs = absf( divide( subtract( ycustom, yexpected ), yexpected ) );
nativeErrs = absf( divide( subtract( ynative, yexpected ), yexpected ) );

console.log( 'The mean relative error of Math.atanh compared to Julia is %d', mean( nativeErrs ) );
