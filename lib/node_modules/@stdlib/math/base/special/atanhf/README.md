...
Computes the [hyperbolic arctangent][hyperbolic-arctangent] of single-precision floating-point number.

```c
float out = stdlib_base_atanhf( 0.0 );
// returns 0.0

out = stdlib_base_atanhf( -0.0 );
// returns -0.0
```
...
int main( void ) {
    const float x[] = { -1.0, -0.78, -0.56, -0.33, -0.11, 0.11, 0.33, 0.56, 0.78, 1.0 };
