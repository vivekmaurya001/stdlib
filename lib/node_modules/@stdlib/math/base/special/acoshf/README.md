...
Computes the [hyperbolic arccosine][hyperbolic-arccosine] of single-precision floating-point number.
...
```c
float out = stdlib_base_acoshf( 1.0 );
// returns 0.0

out = stdlib_base_acoshf( 2.0 );
// returns ~1.317
```
...
int main( void ) {
    const float x[] = { 1.0, 1.45, 1.89, 2.33, 2.78, 3.22, 3.66, 4.11, 4.55, 5.0 };
    
    float v;
    int i;
    for ( i = 0; i < 10; i++ ) {
        v = stdlib_base_acoshf( x[ i ] );
        printf( "acoshf(%f) = %f\n", x[ i ], v );
    }
}
```
